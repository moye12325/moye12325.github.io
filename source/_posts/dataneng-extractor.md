...
summary: >
  ...
  给出两个整数 ，可以对其进行任意次加倍操作。每次操作可以令  或 ，目的是使得最终的  最小（ 的绝对值最小），输出最小绝对值。
  
  输入描述
  
  每个测试文件均包含多组测试数据。
  
  - 第一行输入一个整数  (1 ≤ T ≤ 10^5)，代表数据组数。
  - 每组测试数据描述如下：在一行上输入两个整数  (−10^9 ≤ a, b ≤ 10^9)，代表初始数字。
