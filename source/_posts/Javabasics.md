...
tags:
  - Java
summary: >
  Java 基础包括了很多重要的概念和特性，如平台无关性、面向对象、内存管理、泛型、注解、异常处理、IO 等。Java
  中常用的流有字节流和字符流，字节流适用于处理原始的字节数据，字符流适用于处理文本数据。此外，Java
  中还有序列化和反序列化机制，用于对象的持久化和传输。另外，Java 还支持注解和 SPI 机制，用于实现插件机制和服务提供者接口。Java
  中还存在一些语法糖，如泛型、自动拆装箱、内部类、lambda 表达式等，可以简化代码书写。
---
# Java 基础
