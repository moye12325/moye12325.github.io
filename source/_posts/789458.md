...
tags:
  - 数据结构
summary: "4.1使用栈完成计算功能\n\n  \n\n>  思路分析： > 1. 通过一个index索引遍历表达式 > 2. 发现数字，入数字栈 > 3. 发现是符号： > \t3.1 如果当前的符号栈为空，则直接入栈  > \t3.2 符号栈不为空，有操作符，则进行比较。当前操作符优先级小于等于栈中操作符，则需要从数栈中pop两个数，再从符号栈中pop一个符号，进行运算，并将结果入数字栈，然后将当前操作符入符号栈。\n"
---
# 4.1使用栈完成计算功能
