...
categories:
  - 所遇问题
summary: >
  安装RabbitMQ之后，执行了rabbitmq-plugins enable rabbitmqmanagement，却仍然打不开管理端界面http://localhost:15672
  
  执行了  命令后，仍然无法打开 RabbitMQ 管理端界面，可能是由于以下几个原因造成的：
  
  1. RabbitMQ 服务未启动：确保 RabbitMQ 服务正在运行。可以通过以下命令检查 RabbitMQ 服
---
